/*******************************************************************************
* File Name: LED_Duration_Clock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: LED_Duration_Clock_Start
********************************************************************************
*
* Summary:
...
void LED_Duration_Clock_Start(void) 
{
    ...
    LED_Duration_Clock_CLKEN |= LED_Duration_Clock_CLKEN_MASK;
	LED_Duration_Clock_CLKSTBY |= LED_Duration_Clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: LED_Duration_Clock_Stop
********************************************************************************
*
* Summary:
...
void LED_Duration_Clock_Stop(void) 
{
    ...
    LED_Duration_Clock_CLKEN &= (uint8)(~LED_Duration_Clock_CLKEN_MASK);
	LED_Duration_Clock_CLKSTBY &= (uint8)(~LED_Duration_Clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: LED_Duration_Clock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((LED_Duration_Clock_CLKEN & LED_Duration_Clock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = LED_Duration_Clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = LED_Duration_Clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* LED_Duration_Clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(LED_Duration_Clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(LED_Duration_Clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: LED_Duration_Clock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: LED_Duration_Clock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = LED_Duration_Clock_GetSourceRegister();
    uint16 oldDivider = LED_Duration_Clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = LED_Duration_Clock_CLKEN & LED_Duration_Clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(LED_Duration_Clock_DIV_PTR, clkDivider);
                LED_Duration_Clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                LED_Duration_Clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(LED_Duration_Clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((LED_Duration_Clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(LED_Duration_Clock_DIV_PTR, clkDivider);
				LED_Duration_Clock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: LED_Duration_Clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(LED_Duration_Clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: LED_Duration_Clock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    LED_Duration_Clock_MOD_SRC |= modeBitMask & (uint8)LED_Duration_Clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: LED_Duration_Clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    LED_Duration_Clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(LED_Duration_Clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: LED_Duration_Clock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return LED_Duration_Clock_MOD_SRC & (uint8)(LED_Duration_Clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: LED_Duration_Clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = LED_Duration_Clock_GetDividerRegister();
    uint8 oldSrc = LED_Duration_Clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        LED_Duration_Clock_MOD_SRC |= CYCLK_SSS;
        LED_Duration_Clock_MOD_SRC =
            (LED_Duration_Clock_MOD_SRC & (uint8)(~LED_Duration_Clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: LED_Duration_Clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return LED_Duration_Clock_MOD_SRC & LED_Duration_Clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: LED_Duration_Clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    LED_Duration_Clock_PHASE = clkPhase & LED_Duration_Clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: LED_Duration_Clock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return LED_Duration_Clock_PHASE & LED_Duration_Clock_PHASE_MASK;
}
