...
/*******************************************************************************
* Function Name: Speaker_Clock_2_Start
********************************************************************************
*
* Summary:
...
void Speaker_Clock_2_Start(void) 
{
    ...
    Speaker_Clock_2_CLKEN |= Speaker_Clock_2_CLKEN_MASK;
	Speaker_Clock_2_CLKSTBY |= Speaker_Clock_2_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Speaker_Clock_2_Stop
********************************************************************************
*
* Summary:
...
void Speaker_Clock_2_Stop(void) 
{
    ...
    Speaker_Clock_2_CLKEN &= (uint8)(~Speaker_Clock_2_CLKEN_MASK);
	Speaker_Clock_2_CLKSTBY &= (uint8)(~Speaker_Clock_2_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Speaker_Clock_2_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Speaker_Clock_2_CLKEN & Speaker_Clock_2_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Speaker_Clock_2_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Speaker_Clock_2_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Speaker_Clock_2__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Speaker_Clock_2_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Speaker_Clock_2_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Speaker_Clock_2_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Speaker_Clock_2_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Speaker_Clock_2_GetSourceRegister();
    uint16 oldDivider = Speaker_Clock_2_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Speaker_Clock_2_CLKEN & Speaker_Clock_2_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Speaker_Clock_2_DIV_PTR, clkDivider);
                Speaker_Clock_2_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Speaker_Clock_2_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Speaker_Clock_2_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Speaker_Clock_2_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Speaker_Clock_2_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Speaker_Clock_2_DIV_PTR);
}


/*******************************************************************************
* Function Name: Speaker_Clock_2_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Speaker_Clock_2_MOD_SRC |= modeBitMask & (uint8)Speaker_Clock_2_MODE_MASK;
}


/*******************************************************************************
* Function Name: Speaker_Clock_2_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Speaker_Clock_2_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Speaker_Clock_2_MODE_MASK));
}


/*******************************************************************************
* Function Name: Speaker_Clock_2_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Speaker_Clock_2_MOD_SRC & (uint8)(Speaker_Clock_2_MODE_MASK);
}


/*******************************************************************************
* Function Name: Speaker_Clock_2_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Speaker_Clock_2_GetDividerRegister();
    uint8 oldSrc = Speaker_Clock_2_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Speaker_Clock_2_MOD_SRC |= CYCLK_SSS;
        Speaker_Clock_2_MOD_SRC =
            (Speaker_Clock_2_MOD_SRC & (uint8)(~Speaker_Clock_2_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Speaker_Clock_2_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Speaker_Clock_2_MOD_SRC & Speaker_Clock_2_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Speaker_Clock_2_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Speaker_Clock_2_PHASE = clkPhase & Speaker_Clock_2_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Speaker_Clock_2_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Speaker_Clock_2_PHASE & Speaker_Clock_2_PHASE_MASK;
}
